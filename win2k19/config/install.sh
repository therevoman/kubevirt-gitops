...
oc apply -f windows-2k19-install-vm.yaml
echo "Applied VM"
sleep 5

/tmp/virtctl start windows-2k19-install
echo "Started VM, waiting for VM to start"
sleep 5

vm_ready=$(oc get vm windows-2k19-install -o jsonpath='{.status.ready}')
while [ "$vm_ready" != "true" ]
do
    sleep 10
    vm_ready=$(oc get vm windows-2k19-install -o jsonpath='{.status.ready}')
...
vmi_phase=$(oc get vmi windows-2k19-install -o jsonpath='{.status.phase}')
...
echo "Cleaning up"
oc delete -f windows-2k19-install-vm.yaml

my_app_name=$(oc get cm windows-2k19-install-scripts -o jsonpath='{.metadata.labels.app\.kubernetes\.io/instance}')
